...
def forward_pass(W, xt, yt, h_prev, c_prev, linear = False):
    n = xt.shape[0]
    m = yt.shape[0]
    d = h_prev.shape[0]
    fact = d*n + 2*d**2
    ...
    if not linear:
        ...
        yh = y_temp
        cost = (1/2.0)*np.sum((yh - yt)**2)
    return cost, yh, ht, ct

def cost_function(W, xt, yt, h_prev, c_prev, linear = False):
    n = xt.shape[0]
    m = yt.shape[0]
    d = h_prev.shape[0]
    # print (n,m,d)
    ...
    d_yh = yh - yt # mx1
    d_why = np.dot(d_yh, ht.T) # mx1*1xd => mxd 
    d_by = d_yh # mx1
